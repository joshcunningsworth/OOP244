...
void Crop::out(std::ofstream& ofs)const{
	ofs << length << ' ' << width << ' ' << xpos << ' ' << ypos << ' ';
	p.out(ofs);
}
...
std::ofstream& operator<<(std::ofstream& ofs, const Crop& c){
	c.out(ofs);
	return ofs;
}

std::ifstream& operator>>(std::ifstream& ifs, Crop& c){
	c.in(ifs);
	return ifs;
}
